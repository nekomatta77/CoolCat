...
import { Wallet, Menu, Plus, Minus, Bell } from 'lucide-react';
...
export default function Header({ user, onMenuClick, onDepositClick, onWithdrawClick }: HeaderProps) {
  ...
  const handleBellClick = () => {
    ...
    if (!isNotifOpen) setLocalRead(true);
  };
  ...
  return (
    <header className="bg-white/80 backdrop-blur-xl border-b border-slate-100 px-3 sm:px-4 lg:px-8 py-3 sm:py-4 flex items-center justify-between sticky top-0 z-50">
      ...
      <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
        ...
        {/* Баланс на мобильных - мягкая рамка */}
        ...
        <div className="hidden lg:flex items-center bg-slate-50 border border-slate-100 rounded-[1.2rem] p-1.5 pl-2 gap-3 group hover:bg-white hover:shadow-sm transition-all cursor-default">
          ...
          <div className="flex items-center gap-1.5 border-l border-slate-200/60 pl-3 pr-1">
             ...
             <button 
               ...
               className="w-8 h-8 bg-white border border-slate-200 hover:bg-slate-100 text-slate-600 rounded-xl flex items-center justify-center transition-all active:scale-95 shadow-sm" 
               title="Вывести"
             >
      ...
      <div className="flex items-center gap-2 lg:gap-4 shrink-0">
        ...
        <div className="relative" ref={notifRef}>
          <button 
            ...
            className={cn(
              "p-2 sm:p-2.5 rounded-xl sm:rounded-2xl transition-all relative flex items-center justify-center",
              isNotifOpen ? "bg-brand-50 text-brand-600" : "text-slate-400 hover:text-brand-600 hover:bg-brand-50"
            )}
          >
          ...
          <AnimatePresence>
            {isNotifOpen && (
               <motion.div 
                 ...
                 className="absolute top-full right-0 mt-2 w-[300px] sm:w-[350px] bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 origin-top-right"
               >
