...
export default function Chat({ user, isOpen, setIsOpen }: ChatProps) {
  ...
  // Подписка на сообщения из Firebase
  useEffect(() => {
    const q = query(collection(db, 'messages'), orderBy('createdAt', 'desc'), limit(50));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const msgs = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ChatMessage));
      setMessages(msgs.reverse()); // Переворачиваем, чтобы старые были сверху
    });
    return () => unsubscribe();
  }, []);
  ...
  return (
    <>
      {/* ПЛАВАЮЩАЯ КНОПКА (ТОЛЬКО ДЛЯ ПК) */}
      <button
                  ...
                  {messages.length === 0 ? (
                    ...
                    messages.map((msg) => (
                      <div key={msg.id} className="flex gap-3 animate-in fade-in slide-in-from-bottom-2">
                        <img 
                          src={msg.avatar || '/assets/avatars/ava1.webp'} 
                          alt="avatar" 
                          className="w-10 h-10 rounded-xl object-cover shadow-sm shrink-0 border border-slate-100" 
                        />
                        <div className="flex flex-col">
                          <div className="flex items-baseline gap-2 mb-1">
                            <span className="font-black text-slate-900 text-sm tracking-tight">{msg.nickname}</span>
                            <span className={cn(
