...
export default function Level({ user }: { user: any }) {
  ...
  const currentPoints = isUnranked ? 0 : RANKS[actualRankIndex].points;
  const currentDep = isUnranked ? 0 : RANKS[actualRankIndex].minDeposit;

  const xpProgressPercent = nextRankObj 
    ? Math.min(100, Math.max(0, ((userExp - currentPoints) / (nextRankObj.points - currentPoints)) * 100))
    : 100;

  const depProgressPercent = nextRankObj 
    ? Math.min(100, Math.max(0, ((userDeposits - currentDep) / (nextRankObj.minDeposit - currentDep)) * 100))
    : 100;
      ...
      <section className="bg-white p-6 md:p-8 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/50">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-6">
          <div className="flex items-center gap-4 text-center md:text-left w-full md:w-auto">
            <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center shrink-0", displayRankObj.bg)}>
              <displayRankObj.icon className={cn("w-7 h-7", displayRankObj.color)} />
            </div>
            <div>
              <p className="text-sm font-black uppercase tracking-widest text-slate-400">Твой ранг</p>
              <h2 className="text-2xl font-black text-slate-900">{displayRankObj.name}</h2>
            </div>
          </div>
          ...
          <div className="flex flex-col w-full md:w-[60%] gap-4">
            <div className="space-y-1.5">
              <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-400 px-1">
                <span>Опыт (XP)</span>
                <span className="text-brand-600">{userExp.toLocaleString()} / {nextRankObj ? nextRankObj.points.toLocaleString() : 'MAX'}</span>
              </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-400 px-1">
                <span>Депозит (CAT)</span>
                <span className="text-emerald-500">{userDeposits.toLocaleString()} / {nextRankObj ? nextRankObj.minDeposit.toLocaleString() : 'MAX'}</span>
              </div>
              <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden relative">
                <motion.div 
                  initial={{ width: 0 }}
