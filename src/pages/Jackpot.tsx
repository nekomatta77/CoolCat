...
export default function Jackpot({ user }: JackpotProps) {
  ...
  const activeConfig = ROOMS_CONFIG.find(r => r.id === activeRoomId) || ROOMS_CONFIG[0];
  ...
  const renderCustomAvatar = (player: any, sizeClass: string, innerChildren?: React.ReactNode, mode: 'avatar' | 'card' = 'avatar') => {
    ...
    // Получаем кастомную конфигурацию рамки (scale, x, y)
    const frameConfig = (frameObj as any).config || { scale: 1.35, x: 0, y: 0 };
      ...
      <div className={cn("relative flex-shrink-0 flex items-center justify-center group", sizeClass)}>
        <div 
          className={cn(
            ...
            mode === 'avatar' ? "rounded-[30%] sm:rounded-[35%] border-2 sm:border-4" : "rounded-2xl sm:rounded-4xl border-2 sm:border-[3px]",
            frameObj.css
          )}
          style={{ 
  ...
  return (
    <div className="space-y-4 sm:space-y-8 pb-12 font-sans">
        ...
        {ROOMS_CONFIG.map((roomTab) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative p-3 sm:p-5 rounded-2xl sm:rounded-4xl border text-left transition-all overflow-hidden group active:scale-95",
                isActive 
            ...
            >
              <div className={cn("absolute inset-0 bg-linear-to-br opacity-5 group-hover:opacity-10 transition-opacity", roomTab.gradient)} />
              ...
              {isActive && (
                <div className={cn("absolute bottom-0 left-0 h-1 sm:h-1.5 w-full bg-linear-to-r", roomTab.gradient)} />
              )}
            </button>
          );
        })}
      ...
      {room.history && room.history.length > 0 && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="w-full bg-white border border-slate-100 rounded-3xl sm:rounded-4xl p-3 sm:p-5 shadow-lg shadow-slate-200/40">
            ...
            <AnimatePresence>
              {room.history.map((hist: any, i: number) => (
                <motion.div 
                  ...
                  key={i} 
                  className="flex items-center gap-3 bg-slate-50 border border-slate-100 p-2 sm:p-3 rounded-2xl shrink-0 min-w-50 sm:min-w-60 hover:border-slate-200 transition-colors shadow-sm"
                >
      ...
      <AnimatePresence>
        {errorMessage && (
          <motion.div 
            ...
            className="flex items-center justify-center gap-3 p-3 sm:p-4 bg-linear-to-r from-rose-100/90 to-rose-50/90 border border-rose-200 rounded-xl sm:rounded-2xl text-rose-700 shadow-md shadow-rose-200/50 backdrop-blur-sm mx-auto w-full"
          >
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4 sm:gap-6 items-start">
        ...
        <div className="order-1 lg:order-2 lg:col-span-2 w-full space-y-4 sm:space-y-6">
          <div className="bg-slate-900 rounded-4xl sm:rounded-[2.5rem] p-4 sm:p-8 text-white relative overflow-hidden flex flex-col items-center justify-center min-h-70 sm:min-h-115 border border-slate-800 shadow-2xl">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,var(--tw-gradient-stops))] from-slate-800/40 via-transparent to-transparent -z-10" />
            ...
            {viewMode === 'wheel' && (
              <div className="relative w-52 h-52 sm:w-72 sm:h-72 lg:w-80 lg:h-80 rounded-full border-4 sm:border-8 border-slate-800 flex items-center justify-center shadow-2xl overflow-hidden mt-8 sm:mt-6 bg-slate-950">
                <div className="absolute top-0 z-30 w-0 h-0 border-l-10 sm:border-l-16 border-l-transparent border-r-10 sm:border-r-16 border-r-transparent border-t-18 sm:border-t-28 border-t-rose-500 drop-shadow-[0_4px_4px_rgba(0,0,0,0.5)]" />
                <motion.div 
                  className="w-full h-full rounded-full relative" 
                  initial={{ rotate: 0 }}
                  animate={{ rotate: -wheelRotation }} 
                  transition={{ type: "tween", duration: isAnimating ? 15 : 0, ease: isAnimating ? [0.1, 0.85, 0.15, 1] : "linear" }} 
                  style={{ background: getConicGradient(), willChange: "transform" }}
                >
                  <div className="absolute inset-10 sm:inset-18 bg-slate-900 rounded-full border-4 sm:border-8 border-slate-800 flex flex-col items-center justify-center z-10 shadow-[inset_0_0_20px_rgba(0,0,0,0.8)]">
                    <span className="text-[9px] sm:text-xs font-black text-slate-400 uppercase tracking-widest mt-1">Пул Игры</span>
                    <span className="text-xl sm:text-3xl font-black text-white leading-none my-1">{(room.totalPool || 0).toFixed(0)}</span>
            ...
            {viewMode === 'tape' && (
              <div 
                ...
                className="w-[calc(100%+32px)] sm:w-[calc(100%+64px)] -mx-4 sm:-mx-8 h-30 sm:h-35 bg-slate-950/80 border-y border-slate-800/80 mt-10 sm:mt-12 relative flex items-center shadow-[inset_0_0_40px_rgba(0,0,0,0.8)]" 
              ...
              >
                {room.gameState === 'waiting' || room.gameState === 'countdown' ? (
                        ...
                        return (
                          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} key={idx} className="shrink-0">
                            {renderCustomAvatar(
                              ...
                              <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-slate-950 via-slate-900/80 to-transparent pt-6 pb-1.5 px-1 flex flex-col items-center justify-end z-20">
                  ...
                  <>
                    ...
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-0 h-0 border-l-10 sm:border-l-14 border-l-transparent border-r-10 sm:border-r-14 border-r-transparent border-t-14 sm:border-t-20 border-t-rose-500 z-20 drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]" />
                    <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0 border-l-10 sm:border-l-14 border-l-transparent border-r-10 sm:border-r-14 border-r-transparent border-b-14 sm:border-b-20 border-b-rose-500 z-20 drop-shadow-[0_-2px_4px_rgba(0,0,0,0.5)]" />
                    <motion.div 
              ...
              {showWinnerOverlay && localWinner && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center p-4 z-40">
                  <motion.div 
                    ...
                    className="relative z-10 flex flex-col items-center bg-slate-900 border border-slate-700/50 p-5 sm:p-8 rounded-3xl sm:rounded-4xl shadow-2xl min-w-50 max-w-[85%] sm:max-w-none sm:min-w-85"
                  >
                    ...
                    <div className="relative mb-3 sm:mb-6">
                      ...
                      <div className="relative">
                        ...
                        <div className="absolute -top-3 sm:-top-5 left-1/2 -translate-x-1/2 bg-linear-to-r from-yellow-400 to-amber-500 p-1 sm:p-2 rounded-full border-2 border-slate-900 shadow-xl z-30">
                    ...
                    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-linear-to-r from-emerald-500/20 to-emerald-400/10 border border-emerald-500/30 px-4 sm:px-8 py-1.5 sm:py-3 rounded-xl sm:rounded-2xl backdrop-blur-sm w-full text-center">
                      <span className="text-[8px] sm:text-[10px] text-emerald-200/70 font-black block uppercase tracking-widest mb-0.5 sm:mb-1">Выигрыш (с учетом комиссии)</span>
        ...
        <div className="order-2 lg:order-3 lg:col-span-1 w-full space-y-4 sm:space-y-6">
          <div className="bg-white border border-slate-100 rounded-4xl sm:rounded-[2.5rem] p-4 sm:p-6 shadow-xl shadow-slate-200/40 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] sm:text-xs font-black text-brand-600 bg-brand-50 px-3 py-1 sm:py-1.5 rounded-full border border-brand-100 tracking-widest">
                Общая сумма: {room.totalPool?.toFixed(0) || '0'}
              </span>
            </div>

            <div className="space-y-2 max-h-55 sm:max-h-85 overflow-y-auto custom-scrollbar pr-1">
              <AnimatePresence>
                    ...
                    <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} key={player.uid} className="flex items-center justify-between p-3 sm:p-4 bg-slate-50 border border-slate-100/50 rounded-2xl hover:border-slate-200 transition-colors">
                      <div className="flex items-center gap-3 sm:gap-4">
                        ...
                        <div className="overflow-hidden space-y-0.5">
                          <p className="text-xs sm:text-sm font-bold text-slate-900 tracking-wide truncate max-w-20 sm:max-w-30">{player.nickname}</p>
        ...
        <div className="order-3 lg:order-1 lg:col-span-1 w-full space-y-4 sm:space-y-6">
          <div className="bg-white border border-slate-100 rounded-4xl sm:rounded-[2.5rem] p-4 sm:p-6 shadow-xl shadow-slate-200/40">
            ...
            <div className="space-y-4 sm:space-y-5">
              ...
              <button
                ...
                className={cn(
                  "w-full py-4 sm:py-5 bg-linear-to-r text-white font-black rounded-2xl shadow-xl hover:shadow-2xl hover:-translate-y-0.5 uppercase tracking-widest transition-all active:scale-95 text-sm sm:text-lg flex items-center justify-center gap-2 mt-2",
        ...
        <div className="order-4 lg:order-4 lg:col-span-2 w-full grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <div className="bg-linear-to-br from-amber-50 to-orange-50/50 border border-amber-100 rounded-4xl sm:rounded-[2.5rem] p-4 sm:p-6 shadow-xl relative overflow-hidden flex flex-col">
                  ...
                  <div key={index} className={cn("flex items-center justify-between p-2 sm:p-3 rounded-2xl border backdrop-blur-sm transition-transform hover:scale-[1.02]", bgColors[index] || 'bg-white/50 border-white/50')}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="text-sm sm:text-base font-bold text-slate-800 tracking-wide truncate max-w-20 sm:max-w-30">{top.nickname || top.winner}</span>
          ...
          <div className="bg-linear-to-br from-slate-50 to-white border border-slate-100 rounded-4xl sm:rounded-[2.5rem] p-4 sm:p-6 shadow-xl flex flex-col justify-center items-start">
